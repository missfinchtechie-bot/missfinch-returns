...
function Nav({ step, total, orderNum }: { step?: number; total?: number; orderNum?: string }) {
  return (
    <div className="bg-white border-b border-[#eae7e2] px-4 sm:px-8 py-3 flex items-center justify-between sticky top-0 z-10">
      <span className="text-sm text-[#999]">{orderNum ? `Order #${orderNum}` : ''}</span>
      <img src="/logo-black.png" alt="Miss Finch NYC" className="h-7 sm:h-8" />
      <span className="text-xs text-[#999] tracking-wide">{step && total ? `Step ${step} of ${total}` : ''}</span>
    </div>
  );
}
      ...
      <div className="max-w-[420px] w-full">
        ...
        <div className="bg-white border border-[#eae7e2] rounded-[14px] p-8">
          <div className="text-center mb-7">
            ...
            <p className="text-sm text-[#888] mt-1.5">Find your order to get started</p>
          </div>
          <div className="mb-4">
          ...
          <div className="text-center mt-5 pt-4 border-t border-[#eee]">
            <a href="https://missfinchnyc.com/pages/miss-finch-nyc-return-policy" className="text-xs text-[#999] underline underline-offset-2">View return policy</a>
          </div>
        </div>
      </div>
    </div>
  ...
  return (<><Nav orderNum="8650" />
    <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-8">
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        <div><h1 className="text-2xl font-semibold">Order #8650</h1><p className="text-sm text-[#888] mt-1">Online Store · Apr 2, 2026</p></div>
          ...
          <div className="bg-white border border-[#eae7e2] rounded-[14px] p-5">
            ...
            {[{name:'Noir Garden Midi Dress',size:'M',r:140,p:119},{name:'Indigo Fern Shirt Dress',size:'M',r:178,p:151.30}].map((it,i)=>(
              <div key={i} className="flex gap-3 py-3 border-t border-[#f0ede8]">
                ...
                <div className="flex-1"><div className="text-sm font-medium">{it.name}</div><div className="text-xs text-[#888] mt-0.5">Size: {it.size}</div></div>
                <div className="text-right flex-shrink-0"><div className="text-xs text-[#aaa] line-through">${it.r.toFixed(2)}</div><div className="text-[15px] font-semibold">${it.p.toFixed(2)}</div></div>
              </div>))}
            <div className="pt-3 border-t border-[#f0ede8] text-xs text-[#888]"><span className="font-medium text-[#666]">Carrier:</span> UPS · 1ZK533D50331486061</div>
          </div>
        </div>
        <div className="w-[260px] flex-shrink-0">
          <div className="bg-white border border-[#eae7e2] rounded-[14px] p-5" style={{background:'#FAF9F6'}}>
            ...
            <div className="text-sm space-y-2">
              <div className="flex justify-between"><span className="text-[#888]">Subtotal</span><span>$270.30</span></div>
              <div className="flex justify-between"><span className="text-[#888]">Discounts</span><span className="text-[#D85A30]">-$47.70</span></div>
              <div className="flex justify-between pt-2 border-t border-[#eae7e2] font-semibold"><span>Total</span><span>$270.30</span></div>
            </div>
          </div>
          <div className="text-center mt-4"><div className="text-xs text-[#999]">14-day return window</div><div className="text-xs text-[#0F6E56] font-medium mt-0.5">11 days remaining</div></div>
        </div>
      </div>
    </div></>);
}
...
function SelectItems({ onContinue, onBack }: { onContinue: () => void; onBack: () => void }) {
  ...
  return (<><Nav orderNum="8650" step={1} total={5} />
    <div className="max-w-[680px] mx-auto px-4 sm:px-6 py-8">
      <div className="text-center mb-8"><h1 className="text-[22px] font-medium">Select items to return</h1><p className="text-sm text-[#888] mt-1.5">Choose the items you wish to return or exchange</p></div>
      {items.map((it,i)=>{const a=sel.includes(i);return(
        <div key={i} onClick={()=>toggle(i)} className={`flex gap-3 items-center p-5 bg-white rounded-[14px] mb-3 cursor-pointer transition-all ${a?'border-2 border-[#1a1a1a]':'border border-[#eae7e2]'}`}>
          ...
          <div className="w-[70px] h-[88px] bg-[#f5f3ee] rounded-lg flex-shrink-0" />
          <div className="flex-1"><div className="text-sm font-medium">{it.name}</div><div className="text-xs text-[#888]">Size: {it.size}</div></div>
          <div className="text-right"><div className="text-xs text-[#aaa] line-through">${it.r.toFixed(2)}</div><div className="text-[15px] font-semibold">${it.p.toFixed(2)}</div></div>
        </div>);})}
...
function ReturnReasons({ onContinue, onBack }: { onContinue: () => void; onBack: () => void }) {
  ...
  return (<><Nav orderNum="8650" step={2} total={5} />
    <div className="max-w-[680px] mx-auto px-4 sm:px-6 py-8">
      <div className="text-center mb-8"><h1 className="text-[22px] font-medium">Add return reasons</h1><p className="text-sm text-[#888] mt-1.5">Tell us why you are returning these items</p></div>
      <div className="bg-white border border-[#eae7e2] rounded-[14px] p-5 flex gap-3 items-center">
        <div className="w-[70px] h-[88px] bg-[#f5f3ee] rounded-lg flex-shrink-0" />
        <div className="flex-1"><div className="text-sm font-medium">Noir Garden Midi Dress</div><div className="text-xs text-[#888]">$119.00 / M</div></div>
    ...
    {modal&&<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[14px] max-w-[480px] w-full p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-4">
          <div className="flex gap-3 items-center"><div className="w-[50px] h-[62px] bg-[#f5f3ee] rounded-lg flex-shrink-0" /><div><div className="text-sm font-medium">Noir Garden Midi Dress</div><div className="text-xs text-[#888]">$119.00 / M</div></div></div>
          <button onClick={()=>{if(reason)setModal(false)}} className="text-[#999] text-lg">✕</button>
        </div>
...
function ReturnMethod({ onContinue, onBack }: { onContinue: () => void; onBack: () => void }) {
  ...
  return (<><Nav orderNum="8650" step={3} total={5} />
    <div className="max-w-[780px] mx-auto px-4 sm:px-6 py-8">
      <div className="text-center mb-6"><h1 className="text-[22px] font-medium">How would you like to be compensated?</h1><p className="text-sm text-[#888] mt-1.5">Noir Garden Midi Dress · Size M · $119.00</p></div>
      ...
      <div className="mb-6">
        <div className="text-[11px] font-semibold tracking-wider text-[#aaa] uppercase mb-3">Or exchange for something new</div>
      ...
      <div className="border-t border-[#eae7e2] pt-6">
        <div className="text-[11px] font-semibold tracking-wider text-[#aaa] uppercase mb-3">Or choose a return method</div>
...
function ReviewSubmit({ onSubmit, onBack }: { onSubmit: () => void; onBack: () => void }) {
  return (<><Nav orderNum="8650" step={5} total={5} />
    <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-8">
      <h1 className="text-2xl font-medium mb-7">Review your return</h1>
      <div className="flex gap-8 flex-wrap">
        <div className="flex-1 min-w-[340px]">
          <div className="text-[10px] font-semibold tracking-wider text-[#aaa] uppercase mb-3">Shipping information</div>
          ...
          <div className="text-[10px] font-semibold tracking-wider text-[#aaa] uppercase mb-3">Shipment method</div>
          <div className="text-sm font-medium mb-1">Box and ship it back to us</div><div className="text-sm text-[#888]">Print a shipping label and bring it to a carrier</div>
        </div>
        <div className="w-[300px] flex-shrink-0">
          <div className="bg-white border border-[#eae7e2] rounded-[14px] p-5" style={{background:'#FAF9F6'}}>
            <div className="text-sm font-semibold mb-3">Return items (1)</div>
            <div className="flex gap-2.5 items-center pb-3 border-b border-[#eae7e2]"><div className="w-[50px] h-[62px] bg-[#f0ede8] rounded-lg flex-shrink-0" /><div className="flex-1"><div className="text-sm font-medium">Noir Garden Midi Dress</div><div className="text-[11px] text-[#888]">Size: M</div></div><div className="text-sm font-semibold">$119.00</div></div>
            <div className="pt-3 text-sm space-y-2">
              <div className="flex justify-between"><span className="text-[#888]">Return value</span><span>$119.00</span></div>
              <div className="flex justify-between"><span className="text-[#888]">5% bonus</span><span className="text-[#0F6E56]">+$5.95</span></div>
...
function Confirmation() {
  return (<><Nav />
    <div className="max-w-[680px] mx-auto px-4 sm:px-6 py-12 text-center">
      ...
      <h1 className="text-2xl font-medium">Return submitted</h1><p className="text-sm text-[#888] mt-2">Return #RET-8650-001</p>
      <div className="bg-white border border-[#eae7e2] rounded-[14px] p-6 mt-7 text-left">
        ...
        <p className="text-xs text-[#999] text-center mt-2.5">We&apos;ll also email you a copy</p>
      </div>
      ...
      <a href="https://missfinchnyc.com" className="inline-block mt-6 text-sm text-[#888] underline underline-offset-2">Continue shopping</a>
    </div></>);
}
