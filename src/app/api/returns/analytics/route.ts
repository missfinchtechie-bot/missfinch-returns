...
export async function GET() {
  ...
  const { data: monthly } = await supabase.rpc('get_monthly_returns').select('*');
  
  // Since RPC might not exist, use raw query approach via multiple fetches
  ...
  returns.forEach(r => {
    ...
    const month = r.return_requested.slice(0, 7); // YYYY-MM
  ...
  // Outcome breakdown (for done returns)
  ...
  const rejectionRate = returns.filter(r => r.status === 'done').length > 0
    ? (returns.filter(r => r.outcome === 'rejected').length / returns.filter(r => r.status === 'done').length) * 100 : 0;
  const avgItemsPerReturn = totalReturns > 0 ? returns.reduce((s, r) => s + (r.item_count || 0), 0) / totalReturns : 0;
  
  // Recent 30 days vs prior 30 days
  const now = new Date();
